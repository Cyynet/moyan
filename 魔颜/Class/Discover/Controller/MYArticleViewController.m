...
@interface MYArticleViewController ()<UITableViewDataSource,UITableViewDelegate,UITextViewDelegate,UIGestureRecognizerDelegate>
...
-(void)setuprequestData
{
    ...
    [MYHttpTool postWithUrl:[NSString stringWithFormat:@"%@/publish/queryPublishInfo",kOuternet1] params:params success:^(id responseObject) {
        ...
        if ([responseObject[@"status"] isEqualToString:@"success"])  {
            ...
            self.anwserNumber = responseObject[@"size"];
...
- (void)setHeadData
{
   ...
   [self.iconView setCircleHeaderWithURL:[NSString stringWithFormat:@"%@%@",kOuternet1,self.articleModel.userPic]];
    ...
    sLabel.text = [NSString stringWithFormat:@"共%@个赞,%@条评论",self.articleModel.likeNumber,self.anwserNumber];
